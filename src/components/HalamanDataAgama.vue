<template>
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-4xl font-bold text-yellow-600 mb-8">Data Agama</h1>
    <div class="grid grid-cols-4 gap-6">
      <Card v-for="(religion, index) in filteredReligions" :key="index" :title="religion.title" :count="religion.count" />
    </div>
    <div class="mt-12">
...
export default {
  ...
  methods: {
    createChart() {
      ...
      const labels = filteredData.map(d => `${d.title}: ${((d.count / d3.sum(data)) * 100).toFixed(1)}% ${d.count} Orang`);
      ...
      const color = d3.scaleOrdinal()
        .domain(labels)
        .range(["#ff6347", "#4682b4", "#32cd32", "#ff69b4", "#ff7f50", "#dda0dd", "#8a2be2", "#7fffd4"]); // Warna tambahan untuk setiap kategori
