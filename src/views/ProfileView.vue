<template>
  ...
  <main id="profile" class="flex justify-center items-center px-16 py-20 bg-white max-md:px-5">
    <section class="flex flex-col items-center w-full max-w-[90%] max-md:max-w-full">
      <h1 class="mt-5 text-4xl font-bold text-center text-yellow-primary max-md:max-w-full">
        Wilayah Desa
      </h1>
      <p class="mt-16 text-base text-black max-md:mt-10 max-md:max-w-full mx-24">
      ...
      <div class="mt-16 w-4/5 max-md:mt-10">
        <div class="flex gap-5 max-md:flex-col max-md:gap-0">
          <StatisticBox
            :value="hWilayah"
            label="Hektare Wilayah"
            color="text-red-500"
            statRef="hektareWilayah"
          />
          <StatisticBox
            :value="hSawah"
            label="Hektare Sawah"
            color="text-green-400"
            statRef="hektareSawah"
          />
          <StatisticBox
            :value="hTegalan"
            label="Hektare Tegalan"
            color="text-blue-400"
            statRef="hektareTegalan"
          />
        </div>
      </div>
      <div class="mt-11 max-w-full w-4/5 max-md:mt-10">
        <div class="flex gap-5 max-md:flex-col max-md:gap-0">
          <StatisticBox :value="dusun" label="Dusun" color="text-red-500" statRef="dusun" />
          <StatisticBox :value="rw" label="RW" color="text-green-400" statRef="rw" />
          <StatisticBox :value="rt" label="RT" color="text-blue-400" statRef="rt" />
        </div>
      </div>
      <a
        class="self-stretch mt-28 text-4xl font-bold text-center text-yellow-primary max-md:mt-10 max-md:max-w-full"
        href="/peta-desa"
      >
        Peta Desa
      </a>
      <InteractiveMap loading="lazy" class="mt-10 max-w-full aspect-[2.33] w-[906px]" />
    </section>
  </main>
  <FooterComponent />
</template>
